# Internal course tool HTTP service
...
```json
{"grants":[{"token_sha256":"<sha256 of a randomly generated opaque token>","run_id":"<32 lowercase hex characters>","expires_at":"<RFC3339 expiry>","scopes":["course:read"]}]}
```
...
All three tool routes require `Authorization: Bearer <token>` with a currently valid `course:read` grant. `/healthz` is an unauthenticated health endpoint.
